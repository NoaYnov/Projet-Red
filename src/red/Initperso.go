...
func (p *Personnage) InitPerso() { //fonction de base qui permet à l'utilisateur de choisir sa race, sa classe pour qu'il est des stats et un inventaire adaptées a son envie
	p.mana_max = 150
	p.mana_actuel = 150
	p.initiative = 25
	p.money = 100
	...
	fmt.Println("Bienvenue aventurier! Vous venez de commencer votre aventure dans la région de Tarate plus précisement dans la ville d'Andariel au sud du continent.")
	fmt.Println("Commençons par vous créer un personnage.")
	fmt.Printf("\n")
	fmt.Println("Choisissez votre age")
	fmt.Scanln(&age)
	for i := 1; i > 0; {
		if p.IsNumeric(age) == true {
			p.age = age
			break
		} else {
